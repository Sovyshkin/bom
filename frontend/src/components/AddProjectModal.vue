...
const uploadedFile = ref(null); // Добавляем ref для хранения информации о загруженном файле

const closeModal = () => {
  mainStore.isAddProjectModalOpen = false;
  // Сбрасываем загруженный файл при закрытии модального окна
  uploadedFile.value = null;
  mainStore.excelFile = null; // Очищаем файл из store
};
...
const handleFileSelect = () => {
  // Если файл уже загружен, не показываем диалог выбора файла
...
const handleFileInputChange = (event) => {
  ...
  if (file) {
    console.log('File selected in AddProjectModal:', file);
    uploadedFile.value = file;
    mainStore.excelFile = file; // Сохраняем файл в store
    console.log('File saved to store (excelFile):', mainStore.excelFile);
  }
};
...
const proceed = () => {
  console.log('=== PROCEED CLICKED (AddProjectModal) ===');
  console.log('uploadedFile.value:', uploadedFile.value);
  console.log('mainStore.excelFile:', mainStore.excelFile);
  
  // Проверяем наличие файла в локальной переменной или в store
  let fileToUse = uploadedFile.value || mainStore.excelFile;
  ...
  // ВСЕГДА устанавливаем файл в store перед вызовом
  mainStore.excelFile = fileToUse;
  console.log('=== FILE SET IN STORE ===');
  console.log('mainStore.excelFile after setting:', mainStore.excelFile);
  
  console.log('Calling excelUpload with file...');
  mainStore.excelUpload(fileToUse);
};
...
<template>
  <transition name="modal">
    <div
      v-if="mainStore.isAddProjectModalOpen"
      class="modal-overlay"
      @click="closeModal"
    >
      <div class="modal-content" @click="stopPropagation">
        ...
        <div class="modal-body">
          ...
          >
            ...
            <div class="more-info">
              <p>1. Первый ряд - название колонок</p>
              <p>2. Второй и последующие - данные по каждой позиции</p>
            </div>
          </div>
          ...
          <div class="upload-section">
            ...
            <a class="download-example-btn">Скачать пример таблицы</a>
          </div>
        </div>
      </div>
    </div>
